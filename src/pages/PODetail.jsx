    ...
    <div style={{ marginBottom: 0 }}>
      ...
      {open && items.map((item, idx) => (
        <div key={item.id} style={{
          ...
          borderBottom: idx < items.length - 1 ? 'var(--border-width-1) solid var(--border-default)' : 'none',
          alignItems: 'center',
    ...
    <div className="page-content fade-in">
      ...
      {sections.length > 0 && (
        <div className="p-o-detail-1fac">
          {/* Column headers */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 4.5rem 5.5rem 7.5rem', gap: 'var(--space-s)', padding: 'var(--space-l)', background: 'var(--surface-base)', borderBottom: 'var(--border-width-1) solid var(--border-default)', borderRadius: 'var(--radius-m) var(--radius-m) 0 0' }}>
      ...
      {laborLines.length > 0 && (
        <div className="p-o-detail-1fac">
          ...
          {laborLines.map((line, idx) => (
            <div key={line.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 'var(--space-m)', padding: 'var(--space-m) var(--space-l)', borderBottom: idx < laborLines.length - 1 ? 'var(--border-width-1) solid var(--border-default)' : 'none', alignItems: 'center', background: 'var(--surface-base)' }}>
              <div>
