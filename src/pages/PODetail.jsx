      ...
      {open && items.map((item, idx) => (
        <div key={item.id} style={{
          ...
          <div style={{ minWidth: 0, overflow: 'hidden' }}>
            {item.sku && <div style={{ fontSize: 'var(--text-sm)', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.sku}</div>}
    ...
    <div className="page-content fade-in">
      ...
      <div style={{ background: 'var(--brand-primary)', borderRadius: 'var(--radius-m)', padding: 'var(--space-xl)', marginBottom: 'var(--space-l)', color: '#fff' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-l)',  paddingTop: 'var(--space-m)' }}>
          ...
          {po.job_reference && (
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--surface-base)', fontFamily: 'var(--font-mono)' }}>
      ...
      {sections.length > 0 && (
        <div style={{ background: 'var(--surface-base)', borderRadius: 'var(--radius-m)', overflow: 'hidden', marginBottom: 'var(--space-l)', maxWidth: '100%' }}>
          {/* Column headers */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 4.5rem 5.5rem 7.5rem', gap: 'var(--space-s)', padding: 'var(--space-l)', background: 'var(--surface-base)', borderBottom: '1px solid var(--border-default)', borderRadius: 'var(--radius-m) var(--radius-m) 0 0' }}>
          ...
          {/* Materials subtotal */}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--space-m) var(--space-l)', borderTop: '2px solid var(--border-default)', background: 'var(--surface-hover)' }}>
      ...
      {laborLines.length > 0 && (
        <div style={{ background: 'var(--surface-base)', borderRadius: 'var(--radius-m)', overflow: 'hidden', marginBottom: 'var(--space-l)', maxWidth: '100%' }}>
          ...
          {laborLines.map((line, idx) => (
            <div key={line.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 'var(--space-m)', padding: 'var(--space-m) var(--space-l)', borderBottom: idx < laborLines.length - 1 ? '1px solid var(--border-default)' : 'none', alignItems: 'center', background: 'var(--surface-base)' }}>
              <div>
