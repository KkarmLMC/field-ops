        ...
        <div>
          ...
          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', display: 'flex', gap: 'var(--space-s)', marginTop: 2 }}>
            ...
            {level.quantity_on_order > 0 && (
              <span style={{ color: 'var(--state-info)', fontWeight: 600 }}>+{level.quantity_on_order} on order</span>
            )}
          </div>
        </div>
      </div>
...
function TransactionRow({ tx, warehouseName }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-m)', padding: 'var(--space-m) var(--space-l)', borderBottom: '1px solid var(--border-default)' }}>
      <div style={{
