...
function Card({ title, children }) {
  return (
    <div style={{ background: 'var(--surface-raised)', borderRadius: 'var(--r-xl)', padding: 'var(--sp-4)', border: '1px solid var(--border-l)', marginBottom: 'var(--sp-4)' }}>
      {title && <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 700, marginBottom: 'var(--sp-3)' }}>{title}</div>}
      {children}
    </div>
  )
}
...
function AdvanceForm({ division, projects, onSave, saving, error }) {
  ...
  const inputSm = { width: '100%', fontSize: 'var(--fs-xs)' }
    ...
    <>
      ...
      <Card title="Per Diem">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${mealsTotal.toFixed(2)}</div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 'var(--sp-3)', borderTop: '1px solid var(--border-l)', paddingTop: 'var(--sp-2)' }}>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-2)' }}>Per Diem Total: ${perDiemTotal.toFixed(2)}</span>
        </div>
      </Card>
      ...
      <Card title="Travel Expenses">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-3)' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${hotelTotal.toFixed(2)}</div>
        </div>
        {/* Car rental */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-3)' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${carTotal.toFixed(2)}</div>
        </div>
        {/* Fuel */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-3)' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${parkingTotal.toFixed(2)}</div>
        </div>
        {/* Airline baggage */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-3)' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${baggageTotal.toFixed(2)}</div>
        </div>
        {/* Other lines */}
        {others.map((o, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 'var(--sp-2)', alignItems: 'center', marginBottom: 'var(--sp-2)' }}>
            ...
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, textAlign: 'right' }}>${(o.days * o.rate).toFixed(2)}</div>
          </div>
        ))}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 'var(--sp-3)', borderTop: '1px solid var(--border-l)', paddingTop: 'var(--sp-2)' }}>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-2)' }}>Travel Advance: ${travelTotal.toFixed(2)}</span>
        </div>
      </Card>
      ...
      <div style={{ background: 'var(--navy)', borderRadius: 'var(--r-xl)', padding: 'var(--sp-4) var(--sp-5)', marginBottom: 'var(--sp-4)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 'var(--fs-lg)', fontWeight: 800, color: '#fff' }}>Total Requested</span>
        <span style={{ fontSize: 'var(--fs-2xl)', fontWeight: 800, color: '#fff' }}>${grandTotal.toFixed(2)}</span>
      </div>
      ...
      {error && <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', padding: 'var(--sp-3)', background: '#FEF2F2', borderRadius: 'var(--r-lg)', marginBottom: 'var(--sp-4)', color: '#B91C1C', fontSize: 'var(--fs-sm)' }}><Warning size={14} />{error}</div>}
        ...
        <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
          ...
          {lines.map(line => (
            <div key={line._key} style={{ display: 'grid', gridTemplateColumns: 'minmax(100px,1.5fr) 70px 70px 60px 60px 60px 70px 70px 70px 70px 70px 70px', gap: 4, minWidth: 900, marginBottom: 4 }}>
              <input value={line.vendor} onChange={e => updateLine(line._key, 'vendor', e.target.value)} placeholder="Vendor…" style={{ fontSize: 'var(--fs-xs)', width: '100%' }} />
              {['total','fuel','tolls','parking','car_rental','lodging','meals','supplies','rentals','other'].map(f => (
                <input key={f} type="number" min="0" step="0.01" value={line[f]} onChange={e => updateLine(line._key, f, e.target.value)} style={{ fontSize: 'var(--fs-xs)', width: '100%', textAlign: 'right' }} />
              ))}
              <input type="date" value={line.date} onChange={e => updateLine(line._key, 'date', e.target.value)} style={{ fontSize: 'var(--fs-xs)', width: '100%' }} />
            </div>
          ))}
        </div>
        <button onClick={addLine}
          style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--fs-xs)', fontWeight: 600, color: 'var(--text-3)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 'var(--sp-2)' }}>
          <Plus size={12} /> Add row
        </button>
        {/* Subtotal */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 'var(--sp-3)', borderTop: '1px solid var(--border-l)', paddingTop: 'var(--sp-2)' }}>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-2)' }}>Subtotal: ${subtotal.toFixed(2)}</span>
        </div>
      </Card>
      ...
      <Card title="Mileage Log">
        ...
        {mileage.map(m => (
          <div key={m._key} style={{ display: 'grid', gridTemplateColumns: '1fr 100px', gap: 'var(--sp-2)', marginBottom: 'var(--sp-2)' }}>
            <input type="date" value={m.date} onChange={e => updateMile(m._key, 'date', e.target.value)} style={{ width: '100%', fontSize: 'var(--fs-xs)' }} />
            <input type="number" min="0" value={m.miles} onChange={e => updateMile(m._key, 'miles', e.target.value)} style={{ width: '100%', fontSize: 'var(--fs-xs)', textAlign: 'right' }} />
          </div>
        ))}
        <button onClick={addMile}
          style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--fs-xs)', fontWeight: 600, color: 'var(--text-3)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 'var(--sp-1)' }}>
        ...
        <div style={{ borderTop: '1px solid var(--border-l)', marginTop: 'var(--sp-3)', paddingTop: 'var(--sp-2)', display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-3)' }}>{totalMiles} miles × ${MILEAGE_RATE}/mi</span>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-2)' }}>Mileage: ${mileageTotal.toFixed(2)}</span>
        </div>
      </Card>
      ...
      <div style={{ background: 'var(--surface-raised)', borderRadius: 'var(--r-xl)', overflow: 'hidden', border: '1px solid var(--border-l)', marginBottom: 'var(--sp-4)' }}>
        {[
          ...
          <div key={lbl} style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--sp-3) var(--sp-4)', borderBottom: '1px solid var(--border-l)' }}>
            <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-2)' }}>{lbl}</span>
            <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 700 }}>${val.toFixed(2)}</span>
          </div>
        ))}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--sp-3) var(--sp-4)', borderBottom: '1px solid var(--border-l)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)' }}>
            ...
            <input type="number" min="0" step="0.01" value={lessAdvance} onChange={e => setLessAdvance(e.target.value)}
              style={{ width: 80, textAlign: 'right', fontSize: 'var(--fs-sm)', fontWeight: 700 }} />
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--sp-3) var(--sp-4)', borderBottom: '1px solid var(--border-l)' }}>
          <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-2)' }}>Mileage Reimbursement</span>
          <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 700 }}>${mileageTotal.toFixed(2)}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--sp-4)', background: 'var(--navy)' }}>
          <span style={{ fontSize: 'var(--fs-md)', fontWeight: 800, color: '#fff' }}>Total</span>
          <span style={{ fontSize: 'var(--fs-md)', fontWeight: 800, color: '#fff' }}>${grandTotal.toFixed(2)}</span>
        </div>
      </div>
