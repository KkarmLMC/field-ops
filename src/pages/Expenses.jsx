      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 'var(--sp-3)', marginBottom: 'var(--sp-4)' }}>
        {Object.entries(STATUS).map(([key, s]) => (
          <button key={key} onClick={() => setStatusFilter(statusFilter === key ? 'all' : key)}
            style={{ background: statusFilter === key ? s.bg : 'var(--surface-raised)', borderRadius: 'var(--r-l)', padding: 'var(--sp-3)', border: `1px solid ${statusFilter === key ? s.color + '40' : 'var(--border-l)'}`, cursor: 'pointer', textAlign: 'left' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--sp-2)', marginBottom: 'var(--sp-4)', overflowX: 'auto', scrollbarWidth: 'none' }}>
        {[['all','All'],['advance','Advances'],['expense','Expenses']].map(([val,lbl]) => (
          <button key={val} onClick={() => setTypeFilter(val)}
            style={{ flexShrink: 0, padding: 'var(--sp-1) var(--sp-3)', borderRadius: 'var(--r-full)', border: `1px solid ${typeFilter === val ? 'var(--navy)' : 'var(--border-l)'}`, background: typeFilter === val ? 'var(--navy)' : 'transparent', color: typeFilter === val ? '#fff' : 'var(--black)', fontSize: 'var(--text-xs)', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {lbl}
          </button>
        ))}
      </div>
