        ...
        <div style={{ background:'var(--surface-raised)', borderRadius:'var(--r-xl)', overflow:'hidden', border:'1px solid var(--border-l)', marginBottom:'var(--sp-4)' }}>
          ...
          {pos.map((po, idx) => (
            <button key={idx} onClick={() => navigate(`/sales-orders/${po.id || ''}`)}
              ...
              <span style={{ fontSize:'var(--text-xs)', fontWeight:700, padding:'2px 8px', borderRadius:'var(--r-full)', background: po.status==='submitted'?'#FEF3C7':po.status==='published'?'#EFF6FF':'#F1F5F9', color: po.status==='submitted'?'#D97706':po.status==='published'?'#1D4ED8':'#64748B', textTransform:'capitalize', flexShrink:0 }}>
                {po.status}
              </span>
            </button>
          ))}
