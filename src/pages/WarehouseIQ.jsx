        ...
        <div className="warehouse-i-q-7cb6">
          ...
          {pos.map((po, idx) => (
            <button key={idx} onClick={() => navigate(`/sales-orders/${po.id || ''}`)}
              style={{ width:'100%', display:'flex', alignItems:'center', gap:'var(--space-m)', padding: 'var(--space-s) var(--space-l)', background:'none', cursor:'pointer', textAlign:'left', borderBottom: idx < pos.length-1 ? 'var(--border-width-1) solid var(--border-default)' : 'none' }}>
        ...
        {loading ? (
          ...
          return (
            <div key={r.part.id}
              ...
              style={{ display:'grid', gridTemplateColumns:'1fr 4.5rem 4.5rem 4.5rem 5rem 5.5rem 5.625rem 3rem', gap:'var(--space-s)', padding: 'var(--space-s) var(--space-l)', borderBottom:'var(--border-width-1) solid var(--border-default)', background:rowBg, cursor:'pointer', transition:'background 0.1s' }}
