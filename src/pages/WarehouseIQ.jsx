    ...
    <div className="page-content fade-in">
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'var(--space-l)', flexWrap:'wrap', gap:'var(--space-m)' }}>
        ...
        <div className="flex-gap-s">
          <div style={{ padding: 'var(--space-s) var(--space-l)', borderRadius:'var(--radius-l)', background: isClosed ? 'var(--grey-tint-80)' : 'var(--brand-primary)', color: isClosed ? 'var(--grey-base)' : '#fff', fontWeight:700, fontSize:'var(--text-sm)' }}>
            {periodLabel}
          </div>
          {isClosed
        ...
        <div style={{ background: 'var(--surface-base)', borderRadius:'var(--radius-l)', overflow:'hidden', marginBottom:'var(--space-l)' }}>
          ...
          {pos.map((po, idx) => (
            <button key={idx} onClick={() => navigate(`/sales-orders/${po.id || ''}`)}
              style={{ width:'100%', display:'flex', alignItems:'center', gap:'var(--space-m)', padding: 'var(--space-s) var(--space-l)', background:'none', cursor:'pointer', textAlign:'left', borderBottom: idx < pos.length-1 ? '1px solid var(--border-default)' : 'none' }}>
              ...
              <span style={{ fontSize:'var(--text-xs)', fontWeight:700, padding:'2px 8px', borderRadius:'var(--radius-s)', background: po.status==='queued'?'var(--purple-soft)':po.status==='running'?'var(--warning-soft)':po.status==='fulfillment'?'var(--state-info-soft)':po.status==='shipment'?'var(--blue-tint-80)':'var(--state-success-soft)', color: po.status==='queued'?'var(--purple-tint-20)':po.status==='running'?'var(--warning)':po.status==='fulfillment'?'var(--state-info)':po.status==='shipment'?'var(--blue-shade-20)':'var(--state-success-text)', textTransform:'capitalize', flexShrink:0 }}>
                {po.status}
              </span>
            </button>
          ))}
      ...
      <div style={{ background: 'var(--surface-base)', borderRadius:'var(--radius-l)', overflow:'hidden' }}>
        ...
        {/* Table toolbar */}
        <div style={{ padding: 'var(--space-m) var(--space-l)', borderBottom:'1px solid var(--border-default)', display:'flex', gap:'var(--space-m)', alignItems:'center', flexWrap:'wrap' }}>
          {/* Search */}
        ...
        {loading ? (
          ...
          return (
            <div key={r.part.id}
              ...
              style={{ display:'grid', gridTemplateColumns:'1fr 72px 72px 72px 80px 88px 90px 48px', gap:'var(--space-s)', padding: 'var(--space-s) var(--space-l)', borderBottom:'1px solid var(--border-default)', background:rowBg, cursor:'pointer', transition:'background 0.1s' }}
              onMouseEnter={e => e.currentTarget.style.background = isOut?'var(--error-tint-80)':isLow?'var(--warning-soft)':'var(--surface-hover)'}
              onMouseLeave={e => e.currentTarget.style.background = rowBg}
            >
              {/* Part name + SKU */}
              <div style={{ minWidth:0 }}>
                <div className="text-sm-truncate">{r.part.name}</div>
                {r.part.sku && <div style={{ fontSize:'var(--text-xs)', fontFamily:'var(--font-mono)', color:'var(--text-muted)' }}>{r.part.sku}</div>}
              </div>
              {/* Start */}
              ...
              {/* Used */}
              <div style={{ textAlign:'right', fontSize:'var(--text-sm)', color: r.used > 0 ? 'var(--purple)' : 'var(--text-muted)', fontWeight: r.used > 0 ? 700 : 400 }}>
                {r.used > 0 ? r.used.toLocaleString() : '—'}
              </div>
              {/* On Order */}
              <div style={{ textAlign:'right', fontSize:'var(--text-sm)', color: r.onOrder > 0 ? 'var(--blue-shade-40)' : 'var(--text-muted)', fontWeight: r.onOrder > 0 ? 700 : 400 }}>
        ...
        {/* Totals footer */}
        {filtered.length > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 72px 72px 72px 80px 88px 90px 48px', gap:'var(--space-s)', padding: 'var(--space-m) var(--space-l)', background:'var(--brand-primary)', borderTop:'2px solid var(--border-default)' }}>
