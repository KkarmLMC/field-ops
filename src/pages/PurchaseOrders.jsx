    ...
    <div className="page-content fade-in">
      ...
      {loading ? (
        ...
        <div className="empty">
          ...
          {pos.length === 0 && (
            <button className="btn btn-primary" style={{ marginTop: 'var(--sp-4)' }} onClick={() => navigate('/inventory/purchase-orders/new')}>
              Create First PO
            </button>
          )}
        </div>
        ...
        <div style={{ background: 'var(--surface-raised)', borderRadius: 'var(--r-xl)', overflow: 'hidden' }}>
          {filtered.map(po => (
            <POCard key={po.id} po={po} totals={totals[po.id]} onPress={() => navigate(`/inventory/purchase-orders/${po.id}`)} />
          ))}
        </div>
      )}
    </div>
